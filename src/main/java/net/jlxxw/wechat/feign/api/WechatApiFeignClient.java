...
public interface WechatApiFeignClient {
     ...
     *
     * @param apiDTO 要被清空的账号的appid
     * @param token  依据需要查询的接口属于的账号类型不同而使用不同的token，详情查看注上述注意事项
     *
     * @return <br/>
     * 返回码	errmsg	说明
     * <br/>
    ...
    WeChatResponse openApiClean(@Validated @RequestBody ApiDTO apiDTO,
    ...
    ApiResponse selectQuota(@Validated @RequestBody ApiQuotaDTO apiDTO,
                            @Validated @PathParam("token") String token);
     ...
     */
    @PostMapping("cgi-bin/openapi/quota/get?access_token={token}")
    ApiRequestRecord selectRid(@Validated @RequestBody ApiRidDTO apiDTO,
                               @Validated @PathVariable("token") String token);
}
